...
def gaussian_density(x, norm_factor, dispersion, mean):
    return (1 / (norm_factor * np.sqrt(2*math.pi*dispersion))) * np.exp(-(x - mean)**2 / (2*dispersion))
...
def create_df_from_string(input):
    ...
    number_table = np.array([number_seq[(i-1)*14:i*14] for i in range(1,16 + 1)])
    ...
    second_column_name = [['Mu_k_{}'.format(i), 'V_k_{}'.format(i)] for i in range(1,8)]
    second_column_name = list(chain(*second_column_name))
    
    df = pd.DataFrame(
        index=['Експерт {}'.format(i) for i in range(1,17)], 
        data=number_table,
    ).reset_index()
    
    df.columns = [np.array([''] + first_column_name), np.array([''] + second_column_name)]
